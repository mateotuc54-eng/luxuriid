...
app.post("/api/brand-visual",async(req,res)=>{
  ...
  if(!brand||!request)return res.status(400).json({error:"Le brief est incomplet."});
  if(!client)return res.status(500).json({error:"OPENAI_API_KEY manquante dans .env"});
  const k=kit||{};const prompt=`Create a HIGH-END BRAND IDENTITY BOARD / ART-DIRECTION BOARD for a professional design agency presentation. This is NOT a generic moodboard and NOT a random poster. Build a coherent visual identity system that looks intentionally designed for this exact client.\n\nCLIENT: ${brand}\nSECTOR: ${sector||""}\nSTYLE: ${style||""}\nAUDIENCE: ${audience||""}\nCLIENT REQUEST: ${request}\nDIRECTION: ${directionName||""}\n\nBRAND SYSTEM TO VISUALIZE\nLogo concept: ${k.logoStyle||""}\nMonogram: ${k.monogramStyle||""}\nVisual direction: ${k.visualDirection||""}\nComposition: ${k.composition||""}\nShape language: ${k.shapeLanguage||""}\nContrast: ${k.contrast||""}\nTexture/material: ${k.texture||""}\nPhotography: ${k.photoDirection||""}\nPattern: ${k.pattern||""}\nPalette: ${(k.palette||[]).join(", ")}\nApplications: ${(k.applications||[]).join(", ")}\nAccent use: ${k.accentUse||""}\n\nART DIRECTION\n- Premium editorial layout with a strong grid, generous negative space and clear hierarchy.\n- Show a PRIMARY LOGO prominently, plus 2-3 coherent logo variations / monogram uses.\n- Show the four-color palette as elegant swatches with small HEX labels when legible.\n- Show a typography pairing visually.\n- Show 3-5 realistic applications chosen for the sector: for example packaging, business card, signage, clothing, website, product, vehicle, interior, menu, bottle, box, stationery, social media, etc. Choose only applications that make sense for this client.\n- Include one strong atmospheric photograph or material detail that matches the brand.\n- Keep all visuals consistent: same logo, same palette, same visual language.\n- Use refined materials such as embossed paper, foil, matte surfaces, fabric, glass, wood, metal or digital UI only when appropriate to the sector.\n- Avoid giant circles, random geometric decorations, stock-looking collage elements, childish graphics, clip-art, excessive gradients, clutter and generic luxury clichés.\n- The result should feel like a real brand guideline / identity presentation board made by a top branding studio.\n- If text rendering is imperfect, prioritize the logo mark, layout and visual system over long paragraphs. Keep any supporting text minimal.\n- Do not invent a different brand name or unrelated symbols.`;
  try{const response=await client.images.generate({model:imageModel,prompt,size:"1024x1024",quality:"high"});const b64=response?.data?.[0]?.b64_json;if(!b64)throw new Error("Le visuel IA est vide.");res.json({image:`data:image/png;base64,${b64}`})}catch(e){console.error("VISUAL AI ERROR:",e?.status,e?.code,e?.message||e);res.status(e?.status||500).json({error:e?.message||"La génération visuelle a échoué.",code:e?.code||e?.type||"unknown"})}
});
...
app.post("/api/create-checkout",async(req,res)=>{
  ...
  const {plan="premium",brand="Projet LUXURIID",sector="",request=""}=req.body||{};
  ...
  try{
    ...
    const session=await stripe.checkout.sessions.create({
      ...
      metadata:{plan,brand:brand.slice(0,100),sector:sector.slice(0,100),request:request.slice(0,300)}
    });
...
app.get("/api/checkout/verify",async(req,res)=>{if(!stripe)return res.status(503).json({paid:false,error:"Stripe non configuré"});try{const x=await stripe.checkout.sessions.retrieve(String(req.query.session_id||""));const paid=x.payment_status==="paid";if(paid)rememberOrder({id:x.id,status:x.payment_status,plan:x.metadata?.plan||"",email:x.customer_details?.email||"",createdAt:new Date().toISOString()});res.json({paid,plan:x.metadata?.plan||null,email:x.customer_details?.email||null});}catch(e){res.status(400).json({paid:false,error:"Session de paiement invalide."})}});
