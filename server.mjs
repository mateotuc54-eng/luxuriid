...
async function getPaidCheckoutSession(sessionId){
  if(!stripe) throw Object.assign(new Error("Stripe non configuré."),{status:503});

  const id=String(sessionId||"").trim();

  if(!id){
    throw Object.assign(
      new Error("Paiement requis avant la génération."),
      {status:402}
    );
  }
  ...
  if(session.payment_status!=="paid"){
    throw Object.assign(
      new Error("Paiement non confirmé. La génération reste verrouillée."),
      {status:402}
    );
  }
...
app.post("/api/brand-kit", async (req,res)=>{
  ...
  try{
    paidSession=await getPaidCheckoutSession(session_id);
  }catch(e){
    return res.status(e.status||402).json({
      error:e.message||"Paiement requis."
    });
  }

  const paidMeta=paidSession.metadata||{};

  const finalBrand=String(paidMeta.brand||brand||"Projet LUXURIID");
  const finalSector=String(paidMeta.sector||sector||"");
  const finalStyle=String(paidMeta.style||style||"");
  const finalAudience=String(paidMeta.audience||audience||"");
  const finalRequest=String(paidMeta.request||request||"");
  const finalDirection=String(
    paidMeta.directionName||directionName||"sur mesure"
  );

  if(!finalBrand||!finalRequest){
    return res.status(400).json({
      error:"Le brief est incomplet. Indiquez au minimum le nom de la marque et la demande du client."
    });
  }
  ...
  const schema={
    ...
    properties:{
      ...
      monogramStyle:{type:"string"},
      photoDirection:{type:"string"},
      pattern:{type:"string"},
      accentUse:{type:"string"},
      applications:{
        type:"array",
        minItems:4,
        maxItems:4,
...
BRIEF CLIENT
Marque: ${finalBrand}
Secteur / activité: ${finalSector||"non précisé"}
Style recherché: ${finalStyle||"non précisé"}
Cible: ${finalAudience||"non précisée"}
Demande exacte: ${finalRequest}
Direction choisie: ${finalDirection||"sur mesure"}
...
- Retourne uniquement le JSON du schéma.`;
  ...
  try{
    ...
    res.status(e?.status||500).json({
      error:e?.message||"La génération IA a échoué.",
...
CLIENT
Brand name: ${finalBrand}
Sector / activity: ${finalSector||"not specified"}
Style: ${finalStyle||"not specified"}
Audience: ${finalAudience||"not specified"}
Client brief: ${finalRequest}
Creative direction: ${finalDirection||"custom"}
...
CRITICAL: This must be a single, polished logo presentation. The logo concept and visual language must adapt to the client's actual sector and request. The equestrian example is only a quality reference, never a subject to copy.`;
  ...
  try{
    ...
    if(!b64){
      throw new Error("Le logo IA est vide.");
    }
    ...
    rememberOrder({
      id:paidSession.id,
      status:"paid",
      plan:paidSession.metadata?.plan||"",
      assetId,
      brand:finalBrand,
      createdAt:new Date().toISOString()
    });
    ...
    res.json({
      image:`data:image/png;base64,${previewBuffer.toString("base64")}`,
      assetId
    });
    ...
    res.status(e?.status||500).json({
      error:e?.message||"La génération du logo a échoué.",
      code:e?.code||e?.type||"unknown"
    });
  }
});
...
const PLANS={
  essential:{
    name:"LUXURIID – Identité Essentielle",
    amount:990,
    priceId:process.env.STRIPE_PRICE_ESSENTIAL||""
  },
  premium:{
    name:"LUXURIID – Identité Pro",
    amount:1990,
    priceId:process.env.STRIPE_PRICE_PREMIUM||""
  },
  studio:{
    name:"LUXURIID – Identité Premium",
    amount:2990,
...
app.post("/api/create-checkout",async(req,res)=>{
  if(!stripe){
    return res.status(503).json({
      error:"Paiement Stripe non configuré. Configure STRIPE_SECRET_KEY dans .env."
    });
  }

  const {
    plan="premium",
    brand="Projet LUXURIID",
    sector="",
    style="",
    audience="",
    request="",
  ...
  try{
    const lineItem=p.priceId
      ...
      : {
          price_data:{
            ...
            product_data:{
              name:p.name,
              description:`Branding IA LUXURIID · ${brand}`
            }
          },
          quantity:1
        };
    ...
    const session=await stripe.checkout.sessions.create({
      ...
      metadata:{
        ...
        audience:audience.slice(0,100),
        request:request.slice(0,500),
        directionName:directionName.slice(0,100)
      }
    });
    ...
    res.status(500).json({
      error:e?.message||"Impossible de créer le paiement Stripe."
    });
  }
});
...
app.get("/api/checkout/verify",async(req,res)=>{
  ...
  try{
    ...
    res.json({
      paid,
      plan:x.metadata?.plan||null,
      brand:x.metadata?.brand||"",
      sector:x.metadata?.sector||"",
      style:x.metadata?.style||"",
      audience:x.metadata?.audience||"",
      request:x.metadata?.request||"",
      directionName:x.metadata?.directionName||"",
...
app.get("/api/asset/clean",async(req,res)=>{
  ...
  try{
    ...
    if(
      ...
      session.metadata?.assetId!==assetId
    ){
      return res.status(403).json({
        error:"Paiement non confirmé pour cet export."
      });
    }
    ...
    if(!asset){
      return res.status(410).json({
        error:"Cet export temporaire a expiré. Régénère le logo si nécessaire."
      });
    }
...
app.listen(
  ...
  "0.0.0.0",
  ()=>console.log(
    `LUXURIID AI running on http://localhost:${PORT} · text: ${model} · image: ${imageModel} · stripe: ${stripe?"ON":"OFF"}`
  )
);
